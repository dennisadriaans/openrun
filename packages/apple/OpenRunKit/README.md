...
Shared Apple wire types plus the loopback macOS HTTP client and SSE reader.
The iOS app can reuse the generated types and stream parser, but keeps its
scoped `/api/mobile/**` transport: the versioned `/api/v1/**` API is deliberately
unreachable from another device.

```bash
cd packages/apple/OpenRunKit
swift build
swift test
```

For an app at `apps/ios` in the same repository, use a local path dependency:

```swift
.package(path: "../../packages/apple/OpenRunKit")
```

## What is generated and what is not

`Sources/OpenRunKit/Generated.swift` is produced by `pnpm contract:generate`
from `packages/contracts/src/operations.ts`. **Never edit it** — the next run overwrites
...
- `serverPingInterval` and `staleAfter`, generated from `packages/domain/src/live/protocol.ts`
...
a gate module in `packages/domain/src/`, the server runs it on the read path, and the
...
Response types are not: `packages/runtime/src/core.ts` infers its return types from the
