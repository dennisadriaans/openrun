...
import { resolveCommit } from '../workspaces/git.ts'
import { SETUP_TIMEOUT_MS } from '@openrun/domain/runs/commandBudget'
/**
 * Process executor.
 *
...
import { publishActivityLive } from '../events/activityLive.ts'
import { publishRunLive } from '../events/runLive.ts'
import { pushApprovalRequest, pushApprovalSettled } from '../mobile/apns.ts'
import {
...
import { assertWorkspaceFree, resolveWorkspacePath } from '../workspaces/workspaces.ts'
import { recordRunOutcomeForWorkspace } from '../workspaces/workspaceHealth.ts'
import { missingRunCwdMessage } from '@openrun/domain/workspaces/workspaceHealth'
import {
  shouldCommitUnattendedChanges,
  unattendedCommitFailedMessage,
  unattendedCommitMessage,
} from '@openrun/domain/runs/unattendedCommit'
import { resumesSavedSession, usesFreshExecution } from '@openrun/domain/runs/executionWorkspace'
import { isMessageId } from '@openrun/domain/runs/messageId'
import {
  DEFAULT_RUNTIME_MODE,
  parseRuntimeMode,
  type RuntimeMode,
} from '@openrun/domain/runtimes/runtimeMode'
import type { TurnEventPayload } from '@openrun/domain/chat/turnEvents'
import { assertWorkspaceId } from '@openrun/domain/workspaces/workspaceRef'
import {
  APPROVAL_TIMEOUT_MS,
  assertSupervisedAllowed,
  assertSupervisedSupported,
} from '@openrun/domain/runs/supervisedPolicy'
import { buildControlResponse, type ApprovalDecision } from '@openrun/domain/chat/claudeControl'
import { resolveApprovalAnswer } from '@openrun/domain/runs/approvals'
import type { PermissionOption, ToolKind } from '@openrun/domain/chat/acp'
import { isAcpTransport } from '@openrun/domain/runtimes/acpTransport'
...
import {
  ...
  handoffSystemNote,
  type HandoffMessage,
} from '@openrun/domain/runs/handoffPrompt'
import {
  isRuntimeSwitch,
  resolveSwitchMode,
  resolveSwitchModel,
} from '@openrun/domain/runs/runtimeSwitch'
import { cachedModelsForBin } from '../runtimes/modelCatalog.ts'
import { describeEffectiveTurnSettings } from '@openrun/domain/runs/runSettingsDebug'
import { detectGhFailure, detectGhFailureInEvents } from '@openrun/domain/runs/ghOutcome'
import { assertRuntimeOnPath } from '../runtimes/runtimePath.ts'
import { nativeSessionExists } from '../runtimes/nativeSessions.ts'
import { importNativeTranscript } from '../runtimes/nativeImport.ts'
import {
  missingNativeSessionMessage,
  nativeResumeKindFor,
  nativeResumeNotSupportedMessage,
} from '@openrun/domain/runtimes/nativeSessions'
import { checksForWorkspace, clearCheckPass, runCheckPass } from './checks.ts'
import {
  RUN_KILL_GRACE_MS,
  RUN_STALL_AFTER_MS,
  RUN_STALL_CHECK_MS,
  ...
  runStalledMessage,
  runTimedOutMessage,
} from '@openrun/domain/runs/runBudget'
import { isUsageLimitMessage, usageLimitStopMessage } from '@openrun/domain/runtimes/agentLimits'
import { meaningfulStderr } from '@openrun/domain/runtimes/stderrNoise'
import {
  buildRepairPrompt,
  deriveVerdict,
  shouldAttemptRepair,
  ...
  type RunVerdict,
} from '@openrun/domain/runs/verdict'
import { holdWakeLock, type WakeLock } from '../process/wakeLock.ts'
import {
  agentSpawnOptions,
  isPidAlive,
  isShuttingDown,
  ...
  setShuttingDown,
} from '../process/processControl.ts'
import { peekQueuedMessage, removeQueuedMessage } from './messageQueue.ts'
...
export { latestCheckResults, listCheckResults } from './checks.ts'
