...
const ALLOWED_EDITION_CONSUMERS = new Set([
  'packages/domain/src/cloud/edition.ts',
  'packages/domain/src/cloud/edition.test.ts',
  'packages/domain/src/cloud/sessionEdition.ts',
])
...
function sourceFiles(dir: string, base = dir): Array<string> {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (['node_modules', 'dist', '.tanstack', '.build'].includes(entry.name)) continue
...
test('no local feature is gated behind the edition', () => {
  ...
  for (const relative of ['apps', 'packages'].flatMap((dir) => sourceFiles(join(src, dir), src))) {
