...
import { MOBILE_OPS } from '@openrun/domain/security/mobileScope'
...
describe('portability', () => {
  const files = readdirSync(CONTRACT_DIR, { recursive: true })
    .map(String)
  ...
  for (const file of files) {
    it(`${file} imports nothing that needs a server`, () => {
      ...
      for (const spec of imports) {
        ...
        assert.ok(
          spec.startsWith('.') || spec.startsWith('@openrun/domain/'),
