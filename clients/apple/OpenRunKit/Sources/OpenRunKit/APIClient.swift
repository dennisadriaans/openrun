...
/// Which native surface this versioned client is on.
///
/// The versioned `/api/v1/**` surface is loopback-only, so this client is for
/// a macOS app running beside Open Run. A paired iPhone uses the deliberately
/// narrower `/api/mobile/**` transport instead; changing the base URL cannot
/// turn a versioned route into a mobile route.
