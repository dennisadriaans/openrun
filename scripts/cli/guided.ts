...
import { parseChecks, MAX_CHECK_COMMAND_CHARS } from '../../src/lib/checks.ts'
import {
  resolveRuntime,
  resolveWorkspace,
  type RuntimeChoice,
...
import { isValidCron } from '../../src/lib/cron.ts'
import { pickDefaultRuntime } from '../../src/lib/pickRuntime.ts'
import { RUNTIME_PRESETS } from '../../src/lib/runtimePresets.ts'
...
import { Cancelled, type FlowUi } from './ui.ts'
...
export async function selectRuntime(
  ...
  for (;;) {
    ...
    if (!available.length) {
      ...
      if (next === 'configure') {
        const selected = await manageRuntimes(client, ui)
...
export async function manageRuntimes(
  ...
  if (id === ':exit') return
  if (id === ':add') {
    const presets = RUNTIME_PRESETS.filter((preset) => !rows.some((row) => row.id === preset.id))
    if (!presets.length) {
  ...
  const canUse = runtime.enabled !== 0 && runtime.enabled !== false && runtime.installed !== false
...
export async function chooseSchedule(ui: FlowUi, remote = false): Promise<CliSchedule> {
  ...
  if (choice === 'in' || choice === 'interval') {
    ...
    const day =
      choice === 'week'
        ...
        : choice
    const time = await ui.text('What time?', '09:00', (value) =>
...
export async function guideRun(
  ...
  if (!intent.prompt) intent.prompt = await ui.text('What should the agent do?')
  let runtime = await selectRuntime(client, intent.runtimeHint, ui, !dryRun)
  let workspace = await selectWorkspace(client, intent.workspaceHint, ui, { remote, dryRun })
  if (mode === 'schedule' && intent.schedule.kind === 'now')
    intent.schedule = await chooseSchedule(ui, remote)
  ...
  for (;;) {
    ...
    const action = await ui.select('What next? Enter to continue', [
      {
        value: 'go',
        label: dryRun ? 'Finish preview' : scheduled ? 'Create schedule' : 'Run now',
        hint: 'Enter',
      },
    ...
    if (action === 'schedule') intent.schedule = await chooseSchedule(ui, remote)
    else if (action === 'now') intent.schedule = { kind: 'now' }
    else if (action === 'go') {
      if (scheduled && !dryRun) await ensureProjectChecks(client, workspace, ui)
      ...
      if (setting === 'runtime') {
        const selected = await selectRuntime(client, runtime.id, ui, !dryRun)
        if (selected.id !== runtime.id) intent.modelHint = ''
        runtime = selected
      }
      if (setting === 'workspace')
        workspace = await selectWorkspace(client, workspace.id, ui, { remote, dryRun, force: true })
      if (setting === 'model')
        intent.modelHint = await ui.text(
      ...
      if (setting === 'name')
        intent.name = await ui.text('Automation name', intent.name || deriveTaskName(intent.prompt))
      if (setting === 'pr')
        intent.openPr = await ui.confirm(
