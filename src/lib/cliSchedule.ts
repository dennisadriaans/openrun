...
export type CliIntent = {
  /** What the agent is asked to do. Only interactive drafts may leave this empty. */
  prompt: string
...
function intervalCron(amount: string, unit: string | undefined): string | null {
  ...
  if (MINUTE_UNITS.has(u) && n >= 1 && n <= 60 && 60 % n === 0)
    return n === 60 ? '0 * * * *' : `*/${n} * * * *`
  if (HOUR_UNITS.has(u) && n >= 1 && n <= 24 && 24 % n === 0)
    return n === 24 ? '0 0 * * *' : `0 */${n} * * *`
  return null
}
 ...
 */
export function parseCliSchedule(
  argv: readonly string[],
  now = new Date(),
  /** The interactive CLI fills an omitted prompt after parsing the other choices. */
  options: { allowEmptyPrompt?: boolean } = {},
): CliParse {
  const tokens = argv.filter((t) => t.length > 0)
  if (tokens.length === 0 && !options.allowEmptyPrompt)
  ...
  let literalPrompt: string | undefined
  ...
  function flagValue(token: string, index: number): { value: string; next: number } | null {
    ...
    if (eq !== -1) {
      ...
      return value.trim() ? { value, next: index + 1 } : null
    }
    const next = tokens[index + 1]
    if (next === undefined || next.startsWith('--') || !next.trim()) return null
  ...
  while (i < tokens.length) {
    ...
    if (raw === '--') {
      if (literalPrompt !== undefined)
        return { ok: false, error: 'Use either --prompt or -- to supply the prompt.' }
      literalPrompt = tokens.slice(i + 1).join(' ')
      break
    }
    ...
    if (token === 'every') {
      ...
      if (!result) {
        return {
          ok: false,
          error: `"every ${tokens[i + 1] ?? ''} ${tokens[i + 2] ?? ''}" is not a schedule. Minute intervals must divide 60 evenly; hour intervals must divide 24. Use "cron" for a custom schedule.`,
        }
      }
  ...
  const parsedPrompt = readPromptAndPrIntent(leftover)
  if (explicitNow && schedule)
    return {
      ok: false,
      error: 'Choose either "now" or a schedule. Use --prompt for literal prompt text.',
    }
  if (schedule?.kind === 'once' && (!Number.isFinite(schedule.at) || schedule.at <= now.getTime()))
    return {
      ok: false,
      error:
        'That time has already passed or is out of range. Choose a future time, such as "tomorrow at 9".',
    }
  const prompt = literalPrompt?.trim() ?? parsedPrompt.prompt
  const openPr = literalPrompt === undefined ? parsedPrompt.openPr : false
 ...
 *
 * Tokens containing spaces arrive as literal prompt fragments. Keep other
 * unconsumed words around them in order so a trailing instruction is not lost.
 * The known PR phrases and leading command filler are removed separately.
 */
export function readPromptAndPrIntent(leftover: readonly string[]): {
  ...
  if (quoted.length > 0) {
    ...
    const parts = leftover.filter((token) => /\s/.test(token) || words.includes(token))
    while (parts.length && NOISE.has(parts[0]!.toLowerCase())) parts.shift()
    while (openPr && parts.length && NOISE.has(parts.at(-1)!.toLowerCase())) parts.pop()
