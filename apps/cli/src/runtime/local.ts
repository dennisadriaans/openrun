...
export type LocalStatus = { pid: number; kind: 'worker' | 'web'; home: string; stopping: boolean }
...
export async function localStatus(): Promise<LocalStatus> {
  ...
  for (let attempt = 0; ; attempt++) {
    try {
      return (await localCall('$status')) as LocalStatus
    } catch (error) {
...
export async function ensureLocalRuntime(onStart?: () => void): Promise<CliClient> {
  try {
  ...
  try {
    ...
    const worker = fileURLToPath(
      new URL(source ? '../../../worker/src/index.ts' : './worker.js', import.meta.url),
    )
    const child = spawn(
      ...
      {
        cwd: fileURLToPath(new URL(source ? '../../../../' : '../', import.meta.url)),
