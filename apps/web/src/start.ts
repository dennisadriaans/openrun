...
import {
  securityHeaders,
  shouldSendSecurityHeaders,
} from '@openrun/domain/security/securityHeaders'
...
const accessGuard = createMiddleware({ type: 'request' }).server(async ({ request, next }) => {
  const { accessDecision, withAccessCookie } = await import('@openrun/runtime/security/accessToken')
