...
import type { WorkspaceWithMeta } from '../../fns/index.ts'
import { fetchLatestRunForProject, fetchLatestRunForWorkspace } from '../chat/launchQueries.ts'
import { useConversationNavigationRuns } from '../runs/queries.ts'
import { useProjects } from './queries.ts'
import { truncateBranchLabel } from '@openrun/domain/common/truncateLabel'
import { threadNavigationIndex } from '@openrun/domain/chat/threadLens'
import {
  NavigationProjectPicker,
  NavigationWorkspacePicker,
  type NavigationWorkspace,
} from './NavigationPicker.tsx'
  ...
  const switchTo = async (id: string) => {
    ...
    try {
      ...
      if (latestId) {
        ...
        navigate({
          to: '/runs/new',
          search: { projectId: ws.projectId, workspaceId: ws.id },
        })
      }
    } finally {
      setBusyId(null)
