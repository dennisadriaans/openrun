...
import { isClientAbort } from '@openrun/domain/security/clientAbort'
import { allowRemoteRequest } from '@openrun/domain/security/loopback'
import { lanAllowedHosts } from '@openrun/runtime/mobile/lan'
...
function automationBootstrap(): Plugin {
  return {
    ...
    configureServer(server) {
      ...
      void server.ssrLoadModule('@openrun/runtime/core').catch((error: unknown) => {
...
const config = defineConfig({
  environments: {
    ssr: {
      build: {
        // Node starts this build with type stripping. Keep the runtime in its
        // own package, including native dependencies and process singletons.
        rolldownOptions: { external: /^@openrun\/runtime(?:\/|$)/ },
      },
    },
  },
  define: {
  ...
  resolve: {
    tsconfigPaths: true,
    noExternal: ['@openrun/domain', '@openrun/contracts'],
  },
